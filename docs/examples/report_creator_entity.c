...
int main(int argc, const char ** argv) {
	...
	int rc;
	rc = swid_load_data(io, argv[1], swid);
	swid_destroy_io(io);
	if (rc != 0) {
	...
	for (int i = 0; (entity = swid_get_entity(swid, i)); i++) {
		if (swid_entity_get_role(entity) & SWID_ROLE_SOFTWARE_CREATOR) {
			creator_not_found = 0;
			printf("Creator of %s software: %s [%s]\n", swid_get_name(swid), swid_entity_get_name(entity), swid_entity_get_regid(entity));
			break;
		}
	}
